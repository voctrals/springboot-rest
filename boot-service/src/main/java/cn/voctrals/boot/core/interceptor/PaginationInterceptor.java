...
public class PaginationInterceptor implements Interceptor {
	...
	public Object intercept(final Invocation invocation) throws Throwable {
        ...
        Map<String, Object> pageParameters = (Map<String, Object>)dialect.getParameterObject();
        Map<String, Object> pageMap = null;
        
        List<String> pageParameterKeyList = null;
        boolean isFirst = true;
        for (Map.Entry<String, Object> pageParameterEntry : pageParameters.entrySet()) {
        	Object pageParameterValue = pageParameterEntry.getValue();
        	if (pageParameterValue instanceof PageModel && isFirst) {
        		pageParameterKeyList = new ArrayList<String>();
        		PageModel page = (PageModel)pageParameterValue;
        		Map<Object, Object> map = new BeanMap(page);
        		pageMap = new HashMap<String, Object>();
        		for (Map.Entry<Object, Object> entry : map.entrySet()) {
        			String key = entry.getKey().toString();
        			if (!key.equals("class")) {
        				pageMap.put(key, entry.getValue());
        			}
        		}
        		pageParameterKeyList.add(pageParameterEntry.getKey());
        		isFirst = false;
        	}
        }
        if (pageMap != null) {
        	if (pageParameterKeyList != null) {
        		for (String key : pageParameterKeyList) {
        			pageParameters.remove(key);
        		}
        	}
        	pageParameters.putAll(pageMap);
        }
        
        final BoundSql boundSql = ms.getBoundSql(parameter);

        queryArgs[MAPPED_STATEMENT_INDEX] = copyFromNewSql(ms,boundSql,dialect.getPageSQL(), dialect.getParameterMappings(), pageParameters);
